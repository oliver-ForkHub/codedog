...
export function HomeScreen() {
  ...
  const load = useCallback(async (refresh = false) => {
    ...
    try {
      const [featuredData, latestData, hotData, postData] = await Promise.all([
      ...
      setFeatured(featuredData.slice(0, 10)); setLatest(latestData.list.slice(0, 8));
